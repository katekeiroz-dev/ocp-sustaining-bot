...
class WorkerConfig:
    ...
    ROTA_GROUP_CHANNEL = os.getenv('ROTA_GROUP_CHANNEL', '')  # Channel ID for group notifications
    
    # Team members configuration (from env vars)
    ROTA_LEADS = os.getenv('ROTA_LEADS', '').split(',') if os.getenv('ROTA_LEADS') else []
    ROTA_MEMBERS = os.getenv('ROTA_MEMBERS', '').split(',') if os.getenv('ROTA_MEMBERS') else []
    ...
    SCHEDULE_DM_REMINDER_FRIDAY = os.getenv('SCHEDULE_DM_REMINDER_FRIDAY', '0 17 * * FRI')
    SCHEDULE_DM_REMINDER_MONDAY = os.getenv('SCHEDULE_DM_REMINDER_MONDAY', '0 9 * * MON')
