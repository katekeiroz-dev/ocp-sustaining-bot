...
def get_smartsheet_releases(access_token, sheet_id):
    ...
    print("\n:clipboard: Available columns in sheet:")
            ...
            try:
                ...
                if isinstance(row_data['start_date'], str):
                    ...
                    for date_format in ['%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%Y-%m-%dT%H:%M:%S']:
                        ...
                        print(f":warning:  Could not parse start_date: {row_data['start_date']}")
                        continue
                else:
                ...
                if isinstance(row_data['end_date'], str):
                    for date_format in ['%Y-%m-%d', '%m/%d/%Y', '%d/%m/%Y', '%Y-%m-%dT%H:%M:%S']:
                        ...
                        print(f":warning:  Could not parse end_date: {row_data['end_date']}")
                        continue
                else:
    ...
    print(f"\n:bar_chart: Processed {rows_processed} rows total")
    return releases
...
def main():
    ...
    try:
        ...
        if not validate_credentials(SMARTSHEET_TOKEN, SMARTSHEET_ID):
            print("\n:x: Credential validation failed. Please check your access token and sheet ID.")
            return
