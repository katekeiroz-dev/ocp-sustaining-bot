...
def setup_jobs(scheduler: JobScheduler):
    ...
    if config.ENABLE_DM_REMINDER:
        ...
        logger.info(f"Enabled: ROTA DM reminder - Friday ({config.SCHEDULE_DM_REMINDER_FRIDAY})")
        ...
        logger.info(f"Enabled: ROTA DM reminder - Monday ({config.SCHEDULE_DM_REMINDER_MONDAY})")
    else:
    ...
    logger.info(f"Job setup complete. Total jobs scheduled: {len(scheduler.scheduler.get_jobs())}")
